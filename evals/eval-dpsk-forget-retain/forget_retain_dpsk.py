...
from utils import dpsk_chat, gpt4o_chat
import argparse
...
def evaluate_single_case(case: Dict[str, Any]) -> Dict[str, Any]:
    ...
    llm_response = dpsk_chat(query) # use dpsk_chat or gpt4o_chat
    try:
...
if __name__ == '__main__':
    ...
    if "forget" in args.data_path:
        with open("config/privacy_forget_prompt.txt", "r") as f:
            prompt_template = f.read()
    else:
        with open("config/privacy_retain_prompt.txt", "r") as f:
            prompt_template = f.read()
